...
{
  Waves::State &s = waves.state;
  const Waves::Params &p = waves.params;

  // Handle events.
  const uint32_t flags = s.flags;
  s.flags = Waves::k_flags_none;
  waves.updatePitch(osc_w0f_for_note((params->pitch) >> 8, params->pitch & 0xFF));
  waves.updateWaves(flags);
  ...
  dsp::BiQuad &prelpf = waves.prelpf;
  dsp::BiQuad &postlpf = waves.postlpf;
  q31_t *__restrict y = (q31_t *)yn;
  const q31_t *y_e = y + frames;

  // Sample processor loop.
  for (; y != y_e;)
  {
...
{
  waves.state.flags |= Waves::k_flag_reset;
}
